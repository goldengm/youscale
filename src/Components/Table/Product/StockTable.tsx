...
export default function StockTable({ driverObj }: Props): JSX.Element {
  ...
  const [showDeleteStockModal, setShowDeleteStockModal] =
  ...
  return (
    <TableWrapper
      title="Stock"
      column={["Produit", "Quantite", "Action"]}
      driverObj={driverObj}
      AddBtn={
        <AddStockBtn
    ...
    >
      {showAddStockModal ? (
        createPortal(
          <AddSotckModal
            setIsVisible={setShowAddStockModal}
            refetch={refetch}
            driverObj={driverObj}
          />,
          document.body
      ...
      {showEditStockModal ? (
        createPortal(
          <EditSotckModal
            item={item}
            setIsVisible={setShowEditStockModal}
            refetch={refetch}
          />,
          document.body
        )
      ...
      {showDeleteStockModal ? (
        createPortal(
          <DeleteStockModal
            showModal={showDeleteStockModal}
            item={item}
            setShowModal={setShowDeleteStockModal}
            refetch={refetch}
          />,
          document.body
        )
