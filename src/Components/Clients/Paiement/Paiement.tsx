...
import {
  useAddGoalMutation,
  useGetGoalQuery,
} from "../../../services/api/ClientApi/ClientGoalApi";
...
export default function Paiement({ client }: Props) {
  ...
  const [showDeletePerteModal, setShowDeletePerteModal] =
    useState<boolean>(false);

  const [product, setProduct] = useState<string>("");
  const [date, setDate] = useState<string[]>([]);
  const [usingDate, setUsingDate] = useState<boolean>(false);
  const [OrderQueryData, setOrderQueryData] = useState<DashbordQueryModel>({
  ...
  const driverObj = driver({
    ...
    steps: [
      ...
      {
        element: ".menu-step:nth-child(3)",
        popover: {
          title: "Order page",
          description: "Description for order page",
          side: "right",
          align: "start",
          onPrevClick: (drvHks) => {
            driverObj.moveTo(0);
    ...
    >
      {showAddPerteModal &&
        createPortal(
          <AddPerteModal
            refetch={refetch}
            setShowModal={setShowAddPerteModal}
            showModal={showAddPerteModal}
            driverObj={driverObj}
          />,
          document.body
        )}
      {showDeletePerteModal &&
        createPortal(
          <DeletePerteModal
            refetch={refetch}
            id_perte={String(item?.id) ?? ""}
            setShowModal={setShowDeletePerteModal}
            showModal={showDeletePerteModal}
          />,
          document.body
        )}
        ...
        <div className="container-fluid">
          ...
          <div className="row">
            <Transaction
              ...
              setShowAddPerteModal={setShowAddPerteModal}
              driverObj={driverObj}
              setItem={setItem}
              setShowDeletePerteModal={setShowDeletePerteModal}
            />
  ...
  return (
    <div className="row invoice-card-row">
      <Card
        ...
        value={earning_net}
        title={"gain net"}
        icon={<MdAttachMoney size={35} color={"white"} />}
      />
      <Card
        bg={"success"}
        value={chff_affaire}
        title={"Chiffre d'affaire"}
        icon={<FiShoppingCart size={35} color={"white"} />}
      />
      <Card
        bg={"info"}
        value={spending}
        title={"Spendings"}
        icon={<FaTruckMoving size={35} color={"white"} />}
      />
      ...
      <Card
        bg={"info"}
        value={spending_product}
        title={"Spending Product"}
        icon={<FaTruckMoving size={35} color={"white"} />}
      />
      <Card
        bg={"info"}
      ...
      <Card
        bg={"info"}
        value={spending_commission}
        title={"Spending Commission"}
        icon={<FaTruckMoving size={35} color={"white"} />}
      />
      <Card
        bg={"info"}
        value={spending_landing_design}
        title={"Spending Landing/design"}
        icon={<FaTruckMoving size={35} color={"white"} />}
      />
      <Card
        bg={"info"}
        value={spending_autre}
        title={"Spending Autre"}
        icon={<FaTruckMoving size={35} color={"white"} />}
      />
    </div>
  );
        ...
        <div className="card-body">
          ...
          <div className="d-flex align-items-center">
            <div className="me-auto">
              <div
                className="progress mt-4 goal-progress-line"
                style={{ height: 10 }}
              >
                <div
                  className="progress-bar bg-primary progress-animated"
                  style={{
                    width: `${data?.data.goalPercent || 0}%`,
                    height: 10,
                  }}
                  role="progressbar"
                >
                  <span className="sr-only">
                    `${data?.data.goalPercent || 0}% Complete
                  </span>
                </div>
              </div>
...
const Transaction = ({
  setShowAddPerteModal,
  data,
  setItem,
  setShowDeletePerteModal,
  driverObj,
  ...
  return (
    <div className="col-xl-4 col-xxl-6 col-lg-6">
      <div className="card">
        ...
        <div className="card-body">
          <div id="dlab_W_Todo1" className="widget-media table-paiement">
    ...
    <li>
      <div className="timeline-panel">
        <div className="media-body">
          <h5 className="mb-1">
            {data.Perte_Categorie?.name} - <strong>{data.Product.name}</strong>
          </h5>
          <small className="d-block">
            {data.dateFrom.toString().slice(0, 10)} -{" "}
            {data.dateTo.toString().slice(0, 10)}
          </small>
        </div>
      ...
      <div className="card">
        ...
        <div className="card-body">
          <div id="dlab_W_Todo1" className="widget-media table-paiement">
            <ul className="timeline">
              {data &&
                data.map((dt, index) => <SpendingRow key={index} item={dt} />)}
            </ul>
          </div>
        </div>
      </div>
      ...
      <div className="timeline-panel">
        ...
        <div className="dropdown">
          {item.products.map((pr, key) => (
            <p key={key} className="table-price">
              {pr.name} - {pr.amount}dhs
            </p>
          ))}
        </div>
      </div>
