...
import {
  ...
  DeleteCityModal,
  EditPasswordModal,
} from "../../Table/Modal/Setting";
import {
  useGetStatusQuery,
  usePatchStatusMutation,
} from "../../../services/api/ClientApi/ClientStatusApi";
import {
  CityModel,
  ColumnModel,
  ColumnPatchModel,
...
import {
  CLIENT_STATUS_URL,
  CLIENT_UPLOAD_CITY_URL,
} from "../../../services/url/API_URL";
import { useForm } from "react-hook-form";
import { yupResolver } from "@hookform/resolvers/yup";
import {
  useGetSettingQuery,
  usePatchSettingMutation,
} from "../../../services/api/ClientApi/ClientSettingApi";
...
export default function OrderSetting({ driverObj }: Props) {
  ...
  const [showDeleteCityModal, setShowDeleteCityModal] =
  ...
  return (
    <div className="row">
      {showAddStatusModal && (
        <AddStatusModal
          setShowModal={setShowAddStatusModal}
      ...
      {showAddCityModal && (
        <AddCityModal
          setIsVisible={setShowAddCityModal}
          refetch={refetchData}
          driverObj={driverObj}
        />
      )}
      {showEditCityModal && (
        <EditCityModal
          item={item}
          setIsVisible={setShowEditCityModal}
          refetch={refetchData}
        />
      )}
      {showDeleteCityModal && (
        <DeleteCityModal
          id_city={item?.id ? String(item?.id) : ""}
      ...
      {showEditPasswordModal && (
        <EditPasswordModal
          setShowModal={setShowEditPasswordModal}
      ...
      <Status
        setShowAddStatusModal={setShowAddStatusModal}
  ...
  return (
    <div className={styles.statusContainer}>
      ...
      >
        {statusData &&
          statusData.map((dt) => (
            <StatusRow key={dt.id} dt={dt} refetch={refetchStatus} />
          ))}
      </div>

      <div className={styles.saveStatusBottom}>
        <button type="button" className={styles.apiBtn} onClick={handleSubmit}>
          Enregistrer
        </button>
      </div>
    </div>
...
interface ColumnOfOrderCardProps {
  refetch: () => any;
  objData: {
    active: ColumnModel[] | undefined;
    innactive: ColumnModel[] | undefined;
  };
}
const ColumnOfOrder = ({
  refetch,
  objData,
}: ColumnOfOrderCardProps): JSX.Element => {
  ...
  useEffect(() => {
    setTimeout(() => {
      ...
      innactiveContainer?.addEventListener("dragover", (e) => {
        ...
        const patchData: ColumnPatchModel = {
          id: draggable?.id,
          active: false,
        };
  ...
  return (
    ...
    >
      ...
      <div className="row">
        ...
        />
        <Column
          data={objData.innactive}
          className={"inactive-column"}
          title="Colonnes inactives"
        />
      </div>
      <p className={styles.columnWarning}>
        * : Toutes cases ayant ce symbole ne peuvent pas être inactives{" "}
      </p>
    </div>
  );
};
      ...
      <div className="card-content">
        <div className="nestable">
          <div className="dd" id="nestable">
            <h3
              className={`${styles.nestableTitle} ${
                className == "active-column"
                  ? styles.activeTitle
            ...
            <ol className={`${styles.nestableContainer} ${className}`}>
              {data?.map((dt, index) => (
                <li
                  ...
                  key={index}
                  draggable={true}
                  className={`${styles.nestableItems} col-order`}
                  data-id={index}
                >
...
function DragAndDropFile({ refetch }: DragAndDropFileProps) {
  ...
  const sendData = (data: File) => {
    ...
    axios
      .post(CLIENT_UPLOAD_CITY_URL, formData, {
        headers: {
    ...
    >
      {dragging ? (
        ...
        <>
          ...
          <label htmlFor="payment_image" className="drag-txt">
            Glissez et deposez un fichier excel
          </label>
          <input
            type="file"
            id="payment_image"
  ...
  const onSubmit = (values: Inputs) => {
    const datas = {
      ...
      default_conf_pricing:
        values.default_conf_pricing ||
        String(data?.data.default_cof_ricing) ||
        "0.0",
      delfault_del_pricing:
        values.delfault_del_pricing ||
        String(data?.data.delfaulnpt_del_pricing) ||
        "0.0",
      default_time:
        values.default_time || String(data?.data.default_time) || "0.0",
      startWrldOrder:
        values.startWrldOrder || String(data?.data.startWrldOrder) || "none",
      automated_msg:
        values.automated_msg || String(data?.data.automated_msg) || "nones",
    };
      ...
      <div className={styles.mainCity}>
        ...
        <div className={styles.rightCity}>
          ...
          <p className={styles.cityDesc}>
            Si vous voulez ajouter plusieurs villes en vrac, téléchargez le
            modèle, et déposez le en appuyant sur le bouton ci-dessous.
          </p>
