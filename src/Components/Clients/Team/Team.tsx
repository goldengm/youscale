...
import {
  useLoginAsTeamMutation,
  usePatchClientMutation,
} from "../../../services/api/ClientApi/ClientApi";
...
export default function Team({ client }: Props): JSX.Element {
  ...
  const [idTeam, setIdTeam] = useState<number>(
    GetRole() === "TEAM" ? JSON.parse(userData || "{id: 0}").id : 0
  );
  const [usingDate, setUsingDate] = useState<boolean>(false);
  const [OrderQueryData, setOrderQueryData] = useState<TeamDashbordQueryModel>({
    ...
    steps: [
      {
        ...
        popover: {
          title: "Add your team",
          description: "Add your team here",
          side: "right",
          align: "start",
          onNextClick: (drvHks) => {
            driverObj.moveTo(3);
      ...
      {
        element: ".menu-step:nth-child(4)",
        popover: {
          title: "Product",
          description: "Description for your product page",
          side: "right",
          align: "start",
          onPrevClick: (drvHks) => {
            driverObj.moveTo(0);
...
const TeamRow = ({
  ...
  const handleEditSatus = () => {
    patchTeamMember({ id: item.id, active: !item.active })
      .unwrap()
      .then((res: any) => {
        showToastSucces(
          item?.active ? "Your team has ben hidden" : "Your team has ben showed"
        );
        refetch();
      })
  ...
  const handleLoginAsTeam = (
    e: React.MouseEvent<HTMLAnchorElement, MouseEvent>
  ) => {
    e.preventDefault();

    loginAsTeam({ email: item.email ?? "" })
      .unwrap()
      .then((res: any) => {
...
const EarningTale = ({ earningTable }: EarningTaleProps) => {
  function sumNbCommande(earningTable: EarningTable | undefined) {
    ...
    var sum =
      earningTable.upsell.nb_commande +
      earningTable.livre.nb_commande +
      earningTable.downsell.nb_commande +
      earningTable.crosssell.nb_commande;
    return sum;
  }
  ...
  function sumHerEarning(earningTable: EarningTable | undefined) {
    ...
    var sum =
      earningTable.upsell.her_earning +
      earningTable.livre.her_earning +
      earningTable.downsell.her_earning +
      earningTable.crosssell.her_earning +
      earningTable.crosssell.salaire;
    return sum;
  }
  ...
  function sumSalaire(earningTable: EarningTable | undefined) {
    ...
    var sum =
      earningTable.upsell.salaire +
      earningTable.livre.salaire +
      earningTable.downsell.salaire +
      earningTable.crosssell.salaire;
    return sum;
  }
