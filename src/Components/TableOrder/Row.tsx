...
import {
  ...
  DisplayTeamMember,
  DisplayUpDown,
} from "../Table/Order/OrderRowElement";
...
export default function Row({
  ...
  const handleChangeStatus = (
    ...
    patchOrder({ id: order?.id, status: value })
      .unwrap()
      .then((res) => refetch())
      .then((res) => RefetchStatusData())
      .catch(
  ...
  const handleChangeOuvrir = (
    e: React.ChangeEvent<HTMLSelectElement>
  ): void => {
    const { value } = e.target;

    patchOrder({ id: order?.id, ouvrir: value })
      .unwrap()
      .then(() => refetch && refetch())
      .catch(
        (err: { data: ErrorModel | { message: string }; status: number }) => {
  ...
  const handleChangeChanger = (
    e: React.ChangeEvent<HTMLSelectElement>
  ): void => {
    const { value } = e.target;

    patchOrder({ id: order?.id, changer: value })
      .unwrap()
      .then(() => refetch && refetch())
      .catch(
        (err: { data: ErrorModel | { message: string }; status: number }) => {
  ...
  const handleChangeUpDown = (
    e: React.ChangeEvent<HTMLSelectElement>
  ): void => {
    const { value } = e.target;

    patchOrder({ id: order?.id, updownsell: value })
      .unwrap()
      .then(() => refetch && refetch())
      .catch(
        (err: { data: ErrorModel | { message: string }; status: number }) => {
  ...
  const getRowColor = (
    ...
    const color = statusData.filter((dt) => {
      if (
        dt.name.toUpperCase().replace(" ", "") ==
        currentData.Status.toUpperCase().replace(" ", "")
      ) {
        return dt.color;
      }
    });
      ...
      {column &&
        column.map((dt) => {
          if (dt.active) {
              ...
              return (
                <td key={dt.id}>
                  <img
                    ...
                    src="/svg/order/whatsapp.png"
                    alt="whatsapp"
                    onClick={() => handleClick("+212" + row[formatDtName])}
                  />
                  {order?.telDuplicate && (
                    <BsTelephoneXFill
                      style={{ marginRight: 8 }}
            ...
            // const productOrder = order?.Product_Orders[0];
            // const numOtherOrders = order?.Product_Orders.length - 1;
            ...
            if (formatDtName === "Date")
              return <td key={dt.id}>{row["Date"]}</td>;
