...
const getPageAccess = (
  data: PageAccessModel[] | undefined
): MSEAccessType[] => {
  if (data) {
    var arr: { label: string; value: number }[] = [];
    for (let i = 0; i < data.length; i++) {
      arr.push({
        label: data[i].Client_Page.name,
        value: data[i].Client_Page.id,
      });
    }
...
const getColumnAccess = (
  data: ColumnAccessModel[] | undefined
): MSEAccessType[] => {
  if (data) {
    var arr: { label: string; value: number }[] = [];
    for (let i = 0; i < data.length; i++) {
      arr.push({
        label: data[i].Column_Of_Order.name,
        value: data[i].Column_Of_Order.id,
      });
    }
...
const getProductAccess = (
  data: ProductAccessModel[] | undefined
): MSEAccessType[] => {
  if (data) {
    var arr: { label: string; value: number }[] = [];
    for (let i = 0; i < data.length; i++) {
...
const getCityAccess = (
  data: CityAccessModel[] | undefined
): MSEAccessType[] => {
  if (data) {
    var arr: { label: string; value: number }[] = [];
    for (let i = 0; i < data.length; i++) {
...
const FormBody = ({ refetch, handleClose, dataEdit }: FormBodyProps) => {
  const [showShipping, setShowShipping] = useState<boolean>(
    dataEdit?.livoToken ? true : false
  );
  ...
  const onSubmit = (values: Inputs) => {
    const data: GetTeamMemberModel = {
      ...
      column_access: values.column_access
        ? values.column_access
        : getColumnAccessEdit(dataEdit?.Team_Client_Column_Acces),
      cities_access: values.cities_access
        ? values.cities_access
        : getCityAccessEdit(dataEdit?.Team_Client_City_Acces),
      product_access: values.product_access
        ? values.product_access
        : getProductAccessEdit(dataEdit?.Team_Client_Product_Acces),
      page_access: values.page_access
        ? values.page_access
        : getPageAccessEdit(dataEdit?.Team_Client_Page_Acces),
    };
          ...
          <div className="row">
            <AllAccess
              ...
              selectedValues={getColumnAccess(
                dataEdit?.Team_Client_Column_Acces
              )}
          ...
          <div className="row">
            <AllAccess
              ...
              selectedValues={getProductAccess(
                dataEdit?.Team_Client_Product_Acces
              )}
...
const AllAccess = ({
  ...
  const getArrayFromSelected = (data: []): string[] => {
    var new_arr = data.map((dt: { value: string | number }) =>
      String(dt.value)
    );
